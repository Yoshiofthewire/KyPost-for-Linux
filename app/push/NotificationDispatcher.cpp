...
void NotificationDispatcher::notify(const PushNotification& payload)
{
    ...
    const QString title = !payload.senderName.isEmpty() ? payload.senderName : payload.sender;
    const QString text = !payload.emailSubject.isEmpty() ? payload.emailSubject : payload.subject;
